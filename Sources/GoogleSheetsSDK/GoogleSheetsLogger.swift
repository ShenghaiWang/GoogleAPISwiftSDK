...
public protocol GoogleSheetsLogger: Actor {
...
public actor ConsoleGoogleSheetsLogger: GoogleSheetsLogger {
...
public actor SilentGoogleSheetsLogger: GoogleSheetsLogger {
    public init() {}
...
public actor FileGoogleSheetsLogger: GoogleSheetsLogger {
...
extension GoogleSheetsLogger {
    ...
    /// Log request failure
    public func logRequestFailure(_ context: LoggingContext, error: Swift.Error, duration: TimeInterval? = nil) {
        let actualDuration = duration ?? context.duration()
        var metadata = context.toMetadata()
        metadata["duration"] = String(format: "%.3f", actualDuration)
...
public actor LoggingHTTPClientAdapter: HTTPLogger {
    ...
    public func logRequest(_ request: HTTPRequest) async {
        guard await logger.isEnabled(for: .debug) else { return }
        ...
        await logger.debug("HTTP Request", metadata: metadata)
    }
    
    public func logResponse(_ response: HTTPURLResponse, data: Data) async {
        guard await logger.isEnabled(for: .debug) else { return }
        ...
        await logger.debug("HTTP Response", metadata: metadata)
    }
    
    public func logError(_ error: Swift.Error, for request: HTTPRequest) async {
        ...
        await logger.error("HTTP Error", metadata: metadata)
    }
}
