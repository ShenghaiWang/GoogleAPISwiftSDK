...
public actor GoogleSheetsClient {
    ...
    /// Initialize the client with an API key for read-only operations
    /// - Parameters:
    ///   - serviceAccount: path to Google Servie account json file
    ///   - tokenStorage: the storage for the token, default to `InMemoeryTokenStorage`,
    ///   - httpClient: Optional custom HTTP client (uses default if not provided)
    ///   - logger: Optional logger for debugging and monitoring
    ///   - cache: Optional response cache for read operations
    ///   - cacheConfiguration: Configuration for caching behavior
    ///   - batchOptimizer: Optimizer for batch operations
    ///   - memoryHandler: Handler for memory-efficient operations
    public init(serviceAccount: String,
                ...
                cacheConfiguration: CacheConfiguration = .default,
                batchOptimizer: BatchOptimizer = BatchOptimizer(),
                memoryHandler: MemoryEfficientDataHandler = MemoryEfficientDataHandler()) throws {
        let httpClient =
        if let logger = logger, httpClient == nil {
           URLSessionHTTPClient(logger: LoggingHTTPClientAdapter(logger: logger))
        } else {
    ...
    public func setDebugMode(_ enabled: Bool) async {
        if enabled {
            ...
            await logger?.info("Debug mode disabled")
        }
    }
    
    /// Check if debug mode is enabled
    /// - Returns: true if debug logging is enabled
    public func isDebugModeEnabled() async -> Bool {
        await logger?.isEnabled(for: .debug) ?? false
    }
}
...
extension GoogleSheetsClient {
    ...
    public func readRange(
        ...
        await logger?.logRequestStart(context)
        ...
        do {
            ...
            await logger?.logRequestComplete(context)
            return result
        } catch {
            await logger?.logRequestFailure(context, error: error)
            throw error
        }
    }
    ...
    public func writeStringValues(
        ...
        let anyValues: [[Sendable]] = values.map { row in
            row.map { $0 as Sendable }
        }
...
extension GoogleSheetsClient {
    ...
    public static func isValidA1Range(_ range: String) async -> Bool {
        do {
            _ = try await parseA1Range(range)
            return true
        } catch {
            return false
        }
    ...
    public static func parseA1Range(_ range: String) async throws -> A1Range {
        ...
        return try await valuesService.parseA1Range(range)
    }
...
public struct BatchReadOperation: Sendable {
