...
public actor ValuesService: ValuesServiceProtocol {
    ...
    public func get(spreadsheetId: String, range: String, options: ValueGetOptions? = nil) async throws -> ValueRange {
        ...
        let ranges = try await memoryHandler.splitRangeIntoChunks(range)
    ...
    public func batchGet(spreadsheetId: String, ranges: [String], options: ValueGetOptions? = nil) async throws -> BatchGetValuesResponse {
        ...
        let optimizedRanges = await batchOptimizer.optimizeRanges(ranges)
        let batches = await batchOptimizer.createBatches(from: optimizedRanges)
...
public struct A1Range: Sendable {
